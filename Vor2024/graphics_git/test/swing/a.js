...
var ld = [0.05, 0.7, 0.05];
var bd = [0.8, 0.06, 0.06];
var rd = [0.02, 0.5, 0.02];
var sd = [0.2, 0.02, 0.1];
...
function render() {
	...
	mv1 = mult(mv, translate(bd[0]/2, ld[1]/2, 0.0));
	mv2 = mult(mv1, scalem(ld[0], ld[1], ld[2]));
	...
	mv = mult(mv, translate(0.0, ld[1], 0.0));
	mv1 = mult(mv, scalem(bd[0], bd[1], bd[2]));
	gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
	gl.drawArrays( gl.TRIANGLES, 0, numVertices);

	mv = mult(mv, rotateX(saw));
	mv1 = mult(mv, translate(sd[0]/2, -rd[1]/2, 0.0, ));
	mv1 = mult(mv1, scalem(rd[0], rd[1], rd[2]));
	gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
	gl.drawArrays( gl.TRIANGLES, 0, numVertices);
	...
	mv1 = mult(mv, translate(0.0, -rd[1], 0.0));
	mv1 = mult(mv1, scalem(sd[0], sd[1], sd[2]));
	gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
	gl.drawArrays( gl.TRIANGLES, 0, numVertices);
