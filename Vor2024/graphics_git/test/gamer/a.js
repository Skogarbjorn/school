...
var sd = [0.5, 0.1, 0.5];
var md = [0.04, 0.5, 0.04];
var vd = [0.8, 0.03, 0.03];
...
function render() {
	...
	mv = mult(mv, translate(0.0, sd[1]/2, 0.0));
	mv1 = mult(mv, scalem(sd[0], sd[1], sd[2]));
	gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
	gl.drawArrays( gl.TRIANGLES, 0, numVertices);

	mv = mult(mv, translate(0.0, sd[1]/2 + md[1]/2, 0.0));
	mv = mult(mv, rotateY(rot));
	mv1 = mult(mv, scalem(md[0], md[1], md[2]));
	gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
	gl.drawArrays( gl.TRIANGLES, 0, numVertices);

	mv = mult(mv, translate(0.0, md[1]/2, 0.0));
	mv = mult(mv, rotateZ(saw));
	mv1 = mult(mv, scalem(vd[0], vd[1], vd[2]));
	gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
	gl.drawArrays( gl.TRIANGLES, 0, numVertices);
