...
function bounce_curve(x) {
	var y = Math.sin(x);
	if (y < 0) {
		return y - (1/4)*(Math.pow(y,3));
	}
	return y - (1/5)*Math.pow(y,3);
}
...
var render = function() {
	...
	time = (time + 0.08) % (2*3.14);

	theta[rightLowerLegId] = Math.sin(time) * 30-40;
	theta[leftLowerLegId] = Math.cos(time+3.14/2) * 30-40;
	theta[rightUpperLegId] = Math.sin(time) * 70;
	theta[leftUpperLegId] = Math.cos(time+3.14/2) * 70;
	theta[rightLowerArmId] = Math.sin(time) * 30+60;
	theta[leftLowerArmId] = Math.cos(time+3.14/2) * 30+60;
	theta[rightUpperArmId] = Math.sin(time) * 70 + 180;
	theta[leftUpperArmId] = Math.cos(time+3.14/2) * 70 + 180;
	...
	mv = mult( mv, translate( 0.0, bounce_curve(time*2-0.4), 0.0 ) );
